<?php include_once('includes/url.inc.php'); ?> 
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, minimum-scale=1.0, initial-scale=1, shrink-to-fit=no">
    <link rel="shortcut icon" href="<?php echo $base_url;?>assets/images/favicon.ico" type="image/x-icon"> 
    <title>Teeth Whitening / Bleaching</title>
    
    <!-- All Stylesheets -->
    <?php $folder='root'; include_once('includes/stylesheets.inc.php'); ?> 
</head>
<body class="pt-0"> 
    
    <!-- Preloader, Main Navigation Menu --> 
    <?php $folder='root'; include_once('includes/header.inc.php'); ?> 

    <!-- Header section Start -->
    <section class="page__header">
        <div class="container-fluid">
        
        </div>
    </section>
    <!-- Header section End -->


    <!--Teeth Whitening section Start -->
    <section class="py-4 py-lg-5">
        <div class="container py-xl-4">
            <div class="row g-4 gx-lg-5 align-items-center ">
                <div class="col-lg-5">
                    <img class="img-fluid w-100 object-fit-cover" width="624" height="624" src="assets/images/home/dr-puneet-bhushan.webp" alt="Dr">
                </div><!--//.col-->
                <div class="col-lg-7">
                    <div class="py-lg-4  text-center text-lg-start"> 
                        <h1 class="fs-1 text-primary fw-bold mb-3 text-uppercase">Teeth Whitening / Bleaching at Kalra Dental Care</h1>
                        <p class="fw-bold mb-2">Welcome to a Brighter Smile: Teeth Whitening at Kalra Dental Care</p>
                        <p class="mt-3">
                            Welcome to Kalra Dental Care, where your smile is our pride. Teeth whitening isn't just about a brighter smile; it's about regaining confidence and enjoying the little things in life, like laughing without hesitation. Here, we combine the art and science of dentistry to offer you the best in teeth whitening services.
                        </p>
                    </div>
                </div><!--//.col-->
            </div><!--//.row-->
        </div><!--//.container-->
    </section>
    <!--Teeth Whitening section End -->

   
    <!--extra card section start -->
    <section class="py-2">
        <div class="container">
            <h2 class="fs-2 text-26 text-center fw-bold mb-3">Why Kalra Dental Care for Your Smile Makeover?</h2>
            <p class="mb-4 text-center">
                At Kalra Dental Care, we don’t just whiten teeth; we transform smiles. Our expert team, led by renowned dental specialists, uses the latest techniques and the safest methods to ensure you receive personalized care. Our commitment to excellence means we tailor each treatment to your unique dental profile.
            </p>
            <div class="row g-4">
                <div class="col-lg-4 d-flex">
                    <div class="extra__card w-100">
                        <div class="caption__box py-3 px-4">
                            <h3 class="fs-5 text-41 fw-bold">In-Office Brilliance</h3>
                            <p>
                                Experience the magic of immediate results with our in-office bleaching. Using advanced techniques, we ensure your comfort and safety throughout the process
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 d-flex">
                    <div class="extra__card w-100">
                        <div class="caption__box py-3 px-4">
                            <h3 class="fs-5 text-41 fw-bold">At-Home Convenience</h3>
                            <p>
                                Prefer the comfort of your home? We offer professional-grade home whitening kits, complete with our guidance and supervision.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 d-flex">
                    <div class="extra__card w-100">
                        <div class="caption__box py-3 px-4">
                            <h3 class="fs-5 text-41 fw-bold">In-Office vs. At-Home</h3>
                            <p>
                                Each method has its charm. In-office treatments offer quick, dramatic results, while at-home kits provide flexibility. The choice depends on your lifestyle and preferences.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--extra card section End -->


    <!--Customized Whitening section Start -->
    <section class="py-5">
        <div class="container py-xl-4">
            <div class="row g-4 gx-lg-5 align-items-center ">
                <div class="col-lg-6">
                    <img class="img-fluid w-100 object-fit-cover" width="624" height="485" src="assets/images/about/expertise-and-services.webp" alt="Kalra Dental Care">
                </div><!--//.col-->
                <div class="col-lg-6">
                    <div class="py-lg-4  text-center text-lg-start"> 
                        <h2 class="fs-2 text-26 fw-bold mb-1">Customized Whitening for Every Smile</h2>
                        <p class="mb-2">
                            Sensitive but Bright: For those with sensitive teeth, fear not. We use gentle methods that protect your enamel while delivering stunning results.
                        </p>
                        <p>
                            For the Coffee Aficionados: Love your coffee but not the stains? Our treatments target tough stains, giving you back your brilliant smile without giving up your favorite brew.
                        </p>
                        <h2 class="fs-2 text-26 fw-bold mb-1 mt-4 mt-xxl-5">The Journey to a Whiter Smile</h2>
                        <p class="mb-2">
                            We believe in transparency. Our step-by-step guide walks you through the procedure, from consultation to after-care. We discuss safety, what to expect, and how to maintain your new smile.
                        </p>
                    </div>
                </div><!--//.col-->
            </div><!--//.row-->

            <div class="row g-4 gx-lg-5 align-items-center mt-3 mt-md-4 mt-lg-5">
                <div class="col-lg-6 order-lg-2">
                    <img class="img-fluid w-100 object-fit-cover" width="624" height="485" src="assets/images/about/expertise-and-services.webp" alt="Kalra Dental Care">
                </div><!--//.col-->
                <div class="col-lg-6 order-lg-1">
                    <h2 class="fs-2 text-26 fw-bold mb-1">An Investment in Your Smile</h2>
                    <p class="mt-3">
                        Quality dental care is an investment. We provide a detailed cost breakdown and discuss financing options, ensuring our services are accessible to all.
                    </p>
                    <h2 class="fs-2 text-26 fw-bold mb-1 mt-4 mt-xxl-5">Beyond Whitening: Embracing a Lasting Smile</h2>
                    <p class="mt-3">
                        A bright smile is a journey, not a one-time event. We equip you with tips and tricks for maintaining your radiance long after the treatment, from dietary advice to oral hygiene practices.
                    </p> 
                </div><!--//.col-->
            </div><!--//.row-->

            <div class="row g-4 gx-lg-5 align-items-center mt-3 mt-md-4 mt-lg-5">
                <div class="col-lg-6">
                    <img class="img-fluid w-100 object-fit-cover" width="624" height="485" src="assets/images/about/expertise-and-services.webp" alt="Kalra Dental Care">
                </div><!--//.col-->
                <div class="col-lg-6">
                    <h2 class="fs-2 text-26 fw-bold mb-1">Natural vs. Professional Whitening: A Balanced View</h2>
                    <p class="mt-3">
                        We explore the pros and cons of natural and professional methods. While natural remedies have their place, professional whitening offers safety and efficacy backed by science.
                    </p>
                    <h2 class="fs-2 text-26 fw-bold mb-1 mt-4 mt-xxl-5">Tailoring Your Whitening Experience</h2>
                    <p class="mt-3">
                        Every smile is unique. We offer customized plans based on individual dental profiles, ensuring you receive the most effective treatment.
                    </p> 
                </div><!--//.col-->
            </div><!--//.row-->
        </div><!--//.container-->
    </section>
    <!--Customized Whitening section End -->

    <!-- Promo Section start -->
    <?php 
        $heading="Begin Your Whitening Journey Today";
        $description="Ready for a brighter smile? Contact us to schedule your consultation. Your journey to a dazzling smile begins with a simple step."; 
        include('includes/promo-section.inc.php'); 
    ?>
    <!-- Promo Section End --> 

     <!--Customized Whitening section Start -->
     <section class="pt-5">
        <div class="container pt-lg-4">
            <div class="row g-4 gx-lg-5 align-items-center ">
                <div class="col-lg-6">
                    <img class="img-fluid w-100 object-fit-cover" width="624" height="485" src="assets/images/about/expertise-and-services.webp" alt="Kalra Dental Care">
                </div><!--//.col-->
                <div class="col-lg-6">
                    <div class="py-lg-4  text-center text-lg-start"> 
                        <h2 class="fs-2 text-26 fw-bold mb-3">Dental Insights and Tips</h2>
                        <p class="mb-2">
                            Stay informed with our blog. From dental health tips to the latest in cosmetic dentistry, we keep you updated and engaged.
                        </p>
                        <p>
                            At <strong>Kalra Dental Care</strong>, we're more than just a dental clinic; we're your partners in the journey toward a brighter, healthier smile. Our commitment to excellence, patient-centered care, and comprehensive services make us the go-to destination for all your dental needs. Join us, and let's bring out the best in your smile!
                        </p> 
                    </div>
                </div><!--//.col-->
            </div><!--//.row--> 
        </div><!--//.container-->
    </section>
    <!--Customized Whitening section End -->
  
    
    <!-- FAQ’S section Start -->
    <?php include_once('includes/faq.php'); ?> 
    <!-- FAQ’S section End -->

   <!--Make Appointment section Start -->
   <?php include_once('includes/make-mppointment.php'); ?> 
    <!--Make Appointment section End -->
    
    <!-- Footer and Script List --> 
    <?php $folder='root'; include_once('includes/footer.inc.php'); ?> 
 
 
</body>
</html>
 